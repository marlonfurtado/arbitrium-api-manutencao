...
exports.create = async function(req, res) {
    let validationsErrors = await postValidations(req.body.schedule_id, req.body.week_number, req.body.days);
    if(validationsErrors || !(0 === validationsErrors.length)) { return sendResponse(res, 400, validationsErrors); }
    try {
        const week = new WeekModel({ schedule_id: req.body.schedule_id,  week_number: req.body.week_number });
        let weekResult = await week.save();
        await req.body.days.forEach(async function(day) {
            try {
                ...
                for(let j = 0; j<dayHours.length; j++) {
                    try { const hour = new HourModel({
                            day_id: dayResult.id, activity_id: dayHours[j].activity_id, hour_number: dayHours[j].hour_number });
                        await hour.save();
                    } catch (err) { sendResponse(res, 500, err.message || "Some error occurred while creating the hour."); }}
            } catch (err) { sendResponse(res, 500, err.message || "Some error occurred while creating the day."); }
        });
        ...
        res.send(weekResult);
    } catch(err) { sendResponse(res, 500, err.message || "Some error occurred while creating the week."); }
};
